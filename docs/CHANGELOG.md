...
- 插件配置新增「采用搜索结果条数」（1-10，默认3）
- 插件配置新增「是否启用搜索汇总」开关（默认开启，关闭则直接使用原始搜索结果进入生成阶段）
- 插件配置新增「搜索汇文字数限制」（50-500，默认150）
...
- 删除独立的搜索摘要消息，搜索汇总信息整合到统计信息中展示
- 删除插件配置中的「生成后发送搜索信息摘要」开关
- 搜索汇总改为分析整理后的简短摘要，不再直接展示原始搜索片段
